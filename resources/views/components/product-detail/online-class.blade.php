...
<div>
    ...
    <div class="room-selection">
        ...
        <input {{$room->number_member_maximum - $room->number_member == 0 ? "disabled" : ""}} id="room_{{ $room->id }}"
            type="radio" name="room_id" value="{{$room->id}}" form="form-room">

        <label for="room_{{ $room->id }}" class="pt-3 reviews">
            <div class="row review-bar">
                <div class="col-xl-3 avatar-review">
                    <div>
                        <img src="{{ asset("images/tv.svg")}}" class="img-thumbnail border-0" alt="TV" width="170"
                            height="170">
                    </div>
                ...
                <div
                    class="col-xl-9 {{$room->number_member_maximum - $room->number_member == 0 ? "text-muted" : "text-primary"}}">
                    <p class="h4 font-weight-bold">
                        {{ $courseDetail->course_name }} : {{ $courseDetail->course_description }}
                    </p>
                    <p class="h4 pt-2">
                        {{ Carbon::parse($room->start_date)->format('l') }}｜{{ $room->studySession->session_name }}</p>
                    <p class="h4 pt-2">
