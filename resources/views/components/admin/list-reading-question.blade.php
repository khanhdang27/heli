<ul class="list-group" id="listQuestionReading_{{ $quiz->set }}">
    ...
    <li class="list-group-item d-flex justify-content-between align-items-center list-group-item-info selectable"
        ...
        <div class="w-75 py-5" data-toggle="collapse" data-target="#collapseAnswerReading_{{ $question->id }}"
            aria-expanded="true" aria-controls="collapseAnswerReading_{{ $question->id }}">
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <div class="row">
                            ...
                            <div class="col-6">
                                {!! Form::open(['methods' => 'put', 'url' => route('admin.quiz.question.reading.update',
                                ['quiz' => $quiz->id, 'question' => $question->id]), 'id' => 'formReadingQuestion_{{
                                $question->id }}']) !!}
                                ...
                                <div class="form-group">
                                    ...
                                    {{ Form::text('question', $question->questionContent()->question, ['class' =>
                                    'form-control', 'required']) }}
                                </div>
                                <div class="form-group">
                                    <label for="message_wrong" class="required text-dark">Message Wrong</label>
                                    {{ Form::text('message_wrong', $question->questionContent()->message_wrong, ['class'
                                    => 'form-control', 'required']) }}
                                </div>
                                <div class="form-group">
                                    <label for="lecture_index" class="required text-dark">Lecture Index</label>
                                    {{ Form::text('lecture_index', $question->questionContent()->lecture_index, ['class'
                                    => 'form-control', 'required']) }}
                                </div>
                                <div class="d-flex">
    ...
    <div id="collapseAnswerReading_{{ $question->id }}" class="collapse"
        aria-labelledby="headingQuestion_{{ $question->id }}" data-parent="#listQuestionReading_{{ $quiz->set }}">
        <div class="card-body">
            <div class="d-flex">
            ...
            <!-- Modal -->
            <div class="modal fade" id="modalReadingAnswer{{ $question->id }}" tabindex="-1"
                aria-labelledby="modalReadingAnswer{{ $question->id }}Label" aria-hidden="true">
            ...
            {!! Form::open(['url' => route('admin.quiz.question.reading.setAnswerTrue', ['quiz' => $quiz->id, 'question'
            => $question->questionContent()->id]), 'method' => 'PUT']) !!}
            @csrf
                ...
                <li class="list-group-item d-flex justify-content-between align-items-center">
                    <div class="form-check">
                        <input type="radio" class="form-check-input" id="answer_{{ $item->id }}" name="answer"
                            value="{{ $item->id }}" {{ $item->is_correct == 1 ? 'checked=true' : '' }}>
