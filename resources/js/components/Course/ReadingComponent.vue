        ...
        <div class="text-right w-100 pb-3">
          ...
          <span v-if="typeExam === $getConst('exercise')">
            ...
            <button
              ...
              v-on:click="next()"
              v-if="
                questionIndex < questionReading.length - 1 &&
                resultCheck[questionIndex]
              "
            >
              Next
            </button>
  ...
  methods: {
    ...
    check: function () {
      if (undefined) {
        ...
        for (
          let i = 0;
          i < this.questionReading[this.questionIndex].answers.length;
          i++
        ) {
          if (
            this.questionReading[this.questionIndex].answers[i].id ===
            this.userChoose[this.questionIndex]
          ) {
            if (
              this.questionReading[this.questionIndex].answers[i].is_correct ===
              1
            ) {
