...
class HomeController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        [$course_recommended, $course_hot, $course_welcomes, $course_latest] = $this->getDuplicateCourseByGroupUser();
    ...
    function getCourseHotByTag($course_hot)
    {
        ...
        $courseIGCSE
            ->whereHas('membershipCourses.course.subject.certificate', function ($query) {
                return $query->where('id', 1);
            })
            ->paginate(15);
        ...
        return [$courseIGCSE, $courseIELTS, $courseUKISET, $courseIAL];
    }

    function getDuplicateCourseByGroupUser()
    {
        $courses_with_group = CourseMembershipDiscount::with('membershipCourses', 'courseDiscounts', 'courseDiscounts.discount', 'membershipCourses.course', 'membershipCourses.course.subject', 'membershipCourses.course.subject.certificate', 'membershipCourses.course.tutor', 'membershipCourses.course.courseMaterial')
            ->where('publish', true)
        ...
        $course_welcomes = $course_welcomes
            ...
            ->get();
        $course_latest = $course_latest
            ->whereHas('courseDiscounts.discount', function ($query) {
                return $query->where('start_date', '<=', DATE(NOW()))->where('end_date', '>=', DATE(NOW()));
            })
            ->limit(2)
            ->get();

        return [$course_recommended, $course_hot, $course_welcomes, $course_latest];
    }

    public function wishlist()
    {
        $courses = CourseMembershipDiscount::with('membershipCourses', 'membershipCourses.course', 'membershipCourses.course.likeable', 'membershipCourses.course.likeable.user')
            ->where('publish', true)
